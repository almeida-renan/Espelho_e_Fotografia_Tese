...
# Correlação Candidatos e Eleitos por Partido

# Este script contém o passo a passo para gerar o gráfico 5.10, que mostra
# a relação entre o percentual de candidatos e eleitos não-brancos por partido
...
auto14 <- HC.TSE2014 %>%
  ...
  janitor::tabyl(SG_PARTIDO, DS_COR_RACA) %>%
...
hetero14 <- HC.TSE2014 %>%
  ...
  janitor::tabyl(SG_PARTIDO, Cor.final2) %>%
  janitor::adorn_percentages("row") %>%
  janitor::adorn_pct_formatting(digits = 1, affix_sign = F) %>%
...
#Auto
cand.eleitos.auto14 <- full_join(auto14, auto14.eleitos)
...
#Hetero
cand.eleitos.hetero14 <- full_join(hetero14, hetero14.eleitos)
...
#Auto
cand.eleitos.auto18 <- full_join(auto18, auto18.eleitos)
...
#Hetero
cand.eleitos.hetero18 <- full_join(hetero18, hetero18.eleitos)
...
#Auto
cand.eleitos.auto22 <- full_join(auto22, auto22.eleitos)
...
#Hetero
cand.eleitos.hetero22 <- full_join(hetero22, hetero22.eleitos)
...
# Mudando rótulos de partidos

CE.tudo$SG_PARTIDO <- recode(CE.tudo$SG_PARTIDO,
                             "PATRIOTA" = "PATRI",
                             "SOLIDARIEDADE" = "SD",
                             "REPUBLICANOS" = "REPUB",
                             "CIDADANIA" = "CIDA")
...
CE.tudo %>%
  ggplot(aes(x = Nao.Brancos, y = Não.Brancos.Eleitos, label = SG_PARTIDO)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red", linetype = "dashed") +
  geom_text(vjust = -0.5, size = 4, check_overlap = T) +
  facet_grid(classification ~ Ano) +
  theme_minimal() +
  expand_limits(y = max(CE.tudo$Não.Brancos.Eleitos) * 5) +
...
CE.tudo %>%
  ...
  facet_grid(classification ~ Ano) +
  theme_minimal() +
  ggrepel::geom_text_repel(max.overlaps = 7) +
  labs(x = "Percentual de Candidatos Não-Brancos", y = "Percentual de Eleitos Não-Brancos") +
  theme(strip.text = element_text(size = 13),
        legend.text = element_text(size = 13),
        axis.text.x = element_text(size = 11),  # Tamanho do título do eixo x
...
ggsave("Correlação Cand. e Eleitos Partido.png",bg = "white", dpi = 300, width = 8, height = 6)
