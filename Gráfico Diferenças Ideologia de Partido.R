...
# Gráfico diferença H - A por tamanho de partido

# Este script contém o passo a passo para criar o gráfico 6.4, que mostra
# a diferença em pontos percentuais entre a heteroclassificação e a autodeclaração
# racial de candidatos a deputado federal, por ideologia de partido, nas eleições de 2014, 2018 e 2022.

# Obs: a variável 'Ideologia.partido' é criada no script "Classificação ideológica dos partidos"
...
junto2014 <- junto2014 %>%
  select(Ideologia.partido...1, "Brancos(as)","Pardos(as)", "Pretos(as)")
...
# Gráfico
tudojunto$Ideologia.partido...1 <- factor(tudojunto$Ideologia.partido...1,
                                       levels = c("Esquerda", "Centro", "Direita"))
tudojunto <- tudojunto %>%
  rename(Ideologia.partido = Ideologia.partido...1)
...
tudojunto %>%
  ...
  facet_wrap(vars(Ideologia.partido)) +
  theme_minimal() +
  scale_fill_manual(
...
ggsave("Diferença Hetero-Auto Ideologia.png",bg = "white", dpi = 300, width = 8, height = 6)
