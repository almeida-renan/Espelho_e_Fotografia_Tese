...
# Este script contém o passo a passo para criar o gráfico 6.3, que mostra
# a diferença em pontos percentuais entre a heteroclassificação e a autodeclaração
# racial de candidatos a deputado federal, por tamanho de partido, nas eleições de 2014, 2018 e 2022.

# Obs: a variável 'tamanho.partido' é criada no script "Percentuais raça-cor por tamanho de partido"
...
junto2014 <- junto2014 %>%
  select(tamanho.partido...1, "Brancos(as)","Pardos(as)", "Pretos(as)")
...
tudojunto <- tudojunto %>%
  rename(tamanho.partido = tamanho.partido...1)
...
tudojunto %>%
  ...
  facet_wrap(vars(tamanho.partido)) +
  theme_minimal() +
  scale_fill_manual(
...
ggsave("Diferença Hetero-Auto Tamanho.png",bg = "white", dpi = 300, width = 8, height = 6)
